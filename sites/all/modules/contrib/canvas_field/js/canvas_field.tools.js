 ...
 */
  ...
  */
  ...
  //Pen constructor
  CanvasField.Tools.Line = function() {
    this.lastX = 0;
    this.lastY = 0;
  };
  
  //Extends the baseTool object
  CanvasField.Tools.Line.prototype = new CanvasField.baseTool();

  //Start prototype (get the start coords)
  CanvasField.Tools.Line.prototype.start = function(x, y, context) {
    this.lastX = x;
    this.lastY = y;
  };

  //Move prototype (create the path as we go).
  CanvasField.Tools.Line.prototype.move = function(x, y, context) {
    //If lastX is not set, we must be resuming from a pause.
    if (!this.lastX) {
      this.lastX = x;
      this.lastY = y;
  ...
  //Pause prototype (temporarily stop drawing).
  CanvasField.Tools.Line.prototype.pause = function(x, y, context) {
    this.lastX = 0;
    this.lastY = 0;
  };
